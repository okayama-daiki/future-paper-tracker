...
情報取得・判断の経路は大きく 3 種類ある。

| 経路                   | 説明                                   | 特徴                                               |
| ---------------------- | -------------------------------------- | -------------------------------------------------- |
| **決定論的アダプター** | API・RSS・公式 HTML → ルールベース抽出 | 無料で再現可能。通常の取得経路として最初に使う     |
| **AI エージェント**    | Web 検索・クローリング・LLM による解釈 | 現時点では手動調査時だけ使い、定期実行では使わない |
...
6. **低コスト運用** — 定期実行では有料モデルを呼ばず、GitHub Actions と Pages の範囲で完結させる
...
この節は将来 AI 更新を再導入する場合の設計である。現在の定期実行は confidence を使わず、公式ソースを決定論的に解析できた安全な変更だけを1つの週次コミットとして `main` に反映する。曖昧な情報は変更せず、人間または手動のエージェント/PRフローへ残す。
...
### 6. 決定論的取得を優先するエージェントの更新トリガー戦略

各トリガーでは `config/conference-sources.json` に登録された決定論的アダプターを実行する。日程・締切の証拠を安全に解析できない場合、取得元がエラーになった場合、または複数の公式情報が競合した場合、定期実行は変更せず次回へ持ち越す。Web 検索や LLM は自動フォールバックせず、必要に応じて人間が会議単位の調査フローを起動する。以下の「AI エージェント」は将来の拡張を含む更新オーケストレーターを指し、現在の週次ジョブには含まれない。
